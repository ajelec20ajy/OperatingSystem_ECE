...
found:
  ...
  p->pid = nextpid++;
  p->inuse = 1; // 사용중으로 표시
  p->tickets = 10; // Phase3 기본 value, Phase2에서는 1로했던거같음
  p->ticks = 0;
  release(&ptable.lock);
    ...
    for (;;) {
        ...
        if (total_tickets > 0) {
            ...
            for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
                ...
                // 티켓을 누적하다가 당첨 번호를 넘어설때의  프로세스가 당첨자
                counter += p->tickets;
                if (counter > winner_ticket) {
                    ...
                    p->ticks++; // 이 프로세스의 실행 횟수(ticks) 1 증가
